...
GAZE_COORDS_FRAME_SIZE = (576, 1024)
MAX_SEED = 99999
...
class BatchDataset:
    ...
    def read_gaze_coords(self, data_point_name):
        with open(self.data_dir+'gaze_coords/'+data_point_name+'.pickle', 'rb') as f:
    ...
    def get_annotations_by_coords(self, data_point_names, desired_size=None):
        if desired_size is None:
            desired_size = self.image_size
        annotations = np.zeros((len(data_point_names),) + desired_size,
                               dtype = np.float32)
        for i in range(len(data_point_names)):
            name = data_point_names[i]
            coords = self.read_gaze_coords(name)
            if coords.shape[1] == 0:
                break
            if desired_size is not None and \
            GAZE_COORDS_FRAME_SIZE != desired_size:
                scale = (desired_size[0]/GAZE_COORDS_FRAME_SIZE[0], 
                         desired_size[1]/GAZE_COORDS_FRAME_SIZE[1])
                coords = np.dot(np.diag(scale), coords).astype(np.int)
            for j in range(coords.shape[1]):
                annotations[i, coords[0, j], coords[1, j]] += 1/coords.shape[1]
        annotations = np.expand_dims(annotations, axis=3)
        return annotations
